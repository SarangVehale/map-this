...
def create_crime_rate_map(states_geojson_path, crime_data_csv_path, police_stations_geojson_path, output_file="crime_rate_map.html", log_file="unmatched_entries.log"):
    ...
    Includes the number of crimes reported for each police station.
    """
    ...
    def add_marker_or_circle(location, popup, radius=0, color="blue", fill=True, fill_color="blue", fill_opacity=0.6):
        """
        Adds a marker or circle to the map only if the location is valid.
        """
    ...
    for _, row in state_counts.iterrows():
        ...
        # Get state centroid (average lat/lon of crimes in this state)
        ...
        # Add the state marker or circle
        add_marker_or_circle(
            location=[centroid_lat, centroid_lon],
            popup=folium.Popup(f"<b>State: {state_name}</b><br>Total Crimes: {crime_count}"),
    ...
    for _, row in district_counts.iterrows():
        ...
        # Add the district marker or circle
        add_marker_or_circle(
            location=[centroid_lat, centroid_lon],
            popup=folium.Popup(f"<b>District: {district_name}</b><br>Total Crimes: {crime_count}"),
