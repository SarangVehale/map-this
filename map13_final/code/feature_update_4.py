...
def load_data(states_geojson_path: str, crime_data_csv_path: str, police_stations_geojson_path: str) -> Tuple[Dict, pd.DataFrame, Dict]:
    ...
    try:
        ...
        # Convert columns to string type to prevent sorting issues
...
def create_crime_rate_map(
    ...
    # Add state boundaries with conditional styling
    def style_function(feature):
        if highlight_state and feature['properties'].get('STNAME') == selected_state:
            return {
                'fillColor': '#ff6b6b',
                'color': 'red',
                'weight': 3,
