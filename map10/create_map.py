...
def load_data(database_path="geocoded_data.db"):
    """
    Load geocoded data from an SQLite database. If the table does not exist, create it with sample data.
    ...
    try:
        ...
        if table_exists:
            ...
            print("Warning: Table 'geocoded_locations' does not exist. Creating the table with sample data.")
            # Create the table
            conn.execute("""
            ...
            # Insert sample data
            conn.executemany("""
                INSERT INTO geocoded_locations (latitude, longitude, state, district, police_station, crime_count)
                VALUES (?, ?, ?, ?, ?, ?);
            """, [
                (21.7679, 78.8718, 'Madhya Pradesh', 'Bhopal', 'Bhopal North', 15),
                (19.0760, 72.8777, 'Maharashtra', 'Mumbai', 'Colaba', 30),
                (28.7041, 77.1025, 'Delhi', 'New Delhi', 'Connaught Place', 20)
            ])
            conn.commit()
            df = pd.read_sql_query("SELECT * FROM geocoded_locations", conn)
...
def create_crime_map(data, original_data):
    ...
    crime_map = folium.Map(location=map_center, zoom_start=5, tiles="CartoDB positron")
